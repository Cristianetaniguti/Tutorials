...
# vcfR.obj <- read.vcfR("gatk_filt_maf0.05_maxmissing0.25.recode.vcf")
# DP <- extract.gt(vcfR.obj, "DP")
# vcfR.obj@gt[,-1][which(as.numeric(DP) < 4)] <- "./.:0,0:0:.:0,0,0"
# write.vcf(vcfR.obj, file = "populus_gatk_filt.vcf.gz")
...
However, here we will use `global_error = 0.05` because [`Reads2Map`]() results showed that this option provides the most reliable maps for this data set:

```{r}
onemap_obj_global <- create_probs(onemap.obj.bins, global_error = 0.05)
...
```{r}
onemap_obj_global <- twopts$data.name
