...
2) You have enough local computational resources: use the wrapper [cromwell-cli](https://github.com/lmtani/cromwell-cli)
...
What we like in `Terra.bio`: We consider the most practical way of running a WDL workflow; provides many ways of visualization of the processes; community support
...
## Setup to run locally with `cromwell-cli`

What we like in `cromwell-cli`: easy to install and use;  easy setup of server and Call cache; many options of visualization of the process.

The `cromwell-cli` tool was developed by one of Reads2Map authors (Lucas Taniguti) and provides a easy way to set up, run the workflows, visualize the submitted runs status, and search for results. It requires [`Docker`]().

```{bash, eval=FALSE}
curl https://raw.githubusercontent.com/lmtani/cromwell-cli/main/install.sh | bash

# updated with build
wget https://github.com/lmtani/cromwell-cli/releases/download/v0.10.0/cromwell-cli_Linux_x86_64.tar.gz
tar -xf cromwell-cli_Linux_x86_64.tar.gz

# Get cromwell.jar (check for new releases here: https://github.com/broadinstitute/cromwell/releases)
...
docker run -d --rm --name cromwell-db -p 3307:3306 -e MYSQL_ROOT_PASSWORD=1234 -e MYSQL_DATABASE=cromwell mysql:5.7

docker start cromwell-db

# download configuration file and change according to resources
wget https://github.com/Cristianetaniguti/Reads2Map/raw/develop/.configurations/cromwell_mysql.conf

# start server
java -Dconfig.file=cromwell_mysql.conf -jar cromwell-85.jar server # Run and leave it open in a terminal, use Ctrl+C to exit when you finish to run all your analysis
```

Let the server running in a terminal e open another to submit the jobs as described in the section [`Run workflows`](#Run-workflows)

## Setup to run in High performance computing (HPC) with `Caper`

What we like in `Caper`: do not require MySQL to Call Cache; adaptability to different systems; provide easy way to edit backend in case of HPC specificity; easy to use
...
export PATH=$PATH:/scratch/user/chtaniguti/caper/bin/ # Add executable to the .bashrc file usually located in the home directory
```
...
As I said before, I don't have much space in my home directory, so I opened the `default.conf` file and changed the `local-loc-dir`. I also changed the resources for the leader job, reducing the default time and memory because we are running just a subset in this tutorial.
...
Another change was the time of the leader job because this is a subset, we will not need much time and memory. Check [here](https://github.com/ENCODE-DCC/caper/blob/master/docs/resource_param.md) other options of configuration in `default.conf`.

To pull the singularity images, we will need to have internet access. Another particularity of my HPC is that it requires a specific module to provide internet access to the nodes. Because of these particularity, I needed to modify the backend configurations of caper. They explain how to do it in [this section](https://github.com/ENCODE-DCC/caper/blob/master/DETAILS.md#how-to-override-capers-built-in-backend) of their documentation. Also, because of the space, I need to modify the default directories for singularity cache and images.

I simple added the `module load WebProxy` just before running singularity:
...
## Aspen diploid RADseq example data
...
Edit also the path in `samples_info_pop_sub.tsv`:
...
```{bash, eval=FALSE}
cromwell-cli submit -w EmpiricalSNPCalling_develop.wdl -i EmpiricalSNPCalling_develop.inputs.json -d EmpiricalSNPCalling_develop.zip
```
...
```{bash, eval=FALSE}
cromwell-cli metadata -o 392e2620-5174-4a0d-8ce3-15d71ce52bf4 
```
...
```{bash, eval=FALSE}
cromwell-cli outputs -o 392e2620-5174-4a0d-8ce3-15d71ce52bf4
```
...
```{bash, eval=FALSE}
cromwell-cli navigate -o 392e2620-5174-4a0d-8ce3-15d71ce52bf4
```
...
You can check the workflow log open the file named slurm-<JOBID>. The location of the outputs will be printed at the end of the log file. 
...
```{bash, eval=FALSE}
cromwell-cli submit -w EmpiricalMaps_develop.wdl -i EmpiricalMaps_develop.inputs.json -d EmpiricalMaps_develop.zip
```
...
```{bash, eval=FALSE}
cromwell-cli outputs -o 8e27954a-f477-4db0-a2c9-70f86314914e
```
...
Here are some characteristics that you should check before taking your decision:
...
* **Marker quality**: If all markers are in correct markers but the linkage map has inflated size, it could be due to high error rate amoung the genotypes. Choose one approach that approximates the linkage map size to the size expected (about 100-150 cM if entire chromosome).
...
* **Marker type**: In outcrossing populations, it is important to have markers that have recombination information for both parents. These are the ab x aa (OneMap notation: D1.10), aa x ab (OneMap notation: D2.15), and ab x ab (OneMap notation: B3.7). Avoid approaches that provides only ab x aa or aa x ab in a single chromosome. 
...
Finding these intermediary files can be a bit tricky due to the high number of directories generated. If you are using `cromwell-cli`, the `navigate` function can be useful for that. The graphs generated by `womtools` and a available at the [workflows description documents](https://cristianetaniguti.github.io/Tutorials/Reads2Map/EmpiricalReads.html) are also useful to help you to identify the files location.
...
After all modifications, you will need to adapt the workflow to be run by cromwell-cli or Caper because these tools do not accept imports structure present in Reads2Map. To do that, use:

```{bash, eval=FALSE}
cromwell-cli build -wdl Reads2Map/pipelines/EmpiricalSNPCalling.wdl
```
...
Choose which subworkflow do you want to run in `subworkflows/` and adapt it to run with cromwell-cli and Caper:

```{bash, eval=FALSE}
cromwell-cli build -wdl Reads2Map/subworkflows/gatk_genotyping.wdl
```
