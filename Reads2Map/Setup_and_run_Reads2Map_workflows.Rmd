...
# updated with build
wget https://github.com/lmtani/pumbaa/releases/download/v0.10.0/pumbaa_Linux_x86_64.tar.gz
tar -xf pumbaa_Linux_x86_64.tar.gz
...
Leave the server running in a terminal and open another to submit the jobs as described in the section [`Run workflows`](#Run-workflows)
...
* **Marker quality**: If all markers are in correct order but the linkage map has inflated size, it could be due to high error rate among the genotypes. Choose one approach that approximates the linkage map size to the size expected (about 100-150 cM if entire chromosome).
